...
#include <stdbool.h>  // for true/false (still needed for OLED_Draw_Line call)

// Bit-band macros for direct GPIO pin access
#define GPIOB_ODR_BASE  (GPIOB_BASE + 0x0C)
#define GPIOB_IDR_BASE  (GPIOB_BASE + 0x08)

#define PBout(n) (*((volatile uint32_t *)(PERIPH_BB_BASE + ((GPIOB_ODR_BASE - PERIPH_BASE)*32) + ((n)*4))))
#define PBin(n)  (*((volatile uint32_t *)(PERIPH_BB_BASE + ((GPIOB_IDR_BASE  - PERIPH_BASE)*32) + ((n)*4))))

#define OLED_IIC_SDA    PBout(9)
#define OLED_IIC_SCL    PBout(8)
#define OLED_READ_SDA   PBin(9)

// Yahboom type shortcuts
#ifndef u32
#define u32 uint32_t
#endif
