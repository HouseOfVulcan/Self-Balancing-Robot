...
void OLED_DisplayDebug(float angle, float velocity, int16_t pwm_left, int16_t pwm_right, float battery)
{
    // buffer to be reused throughout
    char buf[22];

    // Angle and Velocity
    snprintf(buf, sizeof(buf), "ANG:%.1f VEL:%.1f", angle, velocity);
    // clear on first line, no refresh yet
    OLED_Draw_Line(buf, 1, true, false);

    // PWM values
    snprintf(buf, sizeof(buf), "L:%d R:%d", pwm_left, pwm_right);
    // no clear, no refresh
    OLED_Draw_Line(buf, 2, false, false);

    // Battery voltage
    snprintf(buf, sizeof(buf), "BAT:%.1fV", battery);
    // last line, trigger refresh
